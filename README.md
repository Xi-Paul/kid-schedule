...
## 3-7. 성취 표시와 음성 — 지치지 않게 만드는 규칙

기능을 얹을수록 앱이 잔소리꾼이 되기 쉽습니다. 아래 규칙을 코드에 박아 뒀습니다.

### 성취

- 헤더에 **오늘 진행 링**과 배지(🎉 계획 완주 / ⏱ 전부 시간 안에 / 🌱 스스로 한 것 / 📝 돌아보기)
- 마지막 하나를 채우면 **하루 한 번** 축하 화면. 되돌렸다 다시 해도 또 뜨지 않습니다
- **놓친 항목은 진행 링의 분모에서 뺍니다.** 아침에 하나 놓쳤다고 그날이 "이미 망한 날"이 되면 안 됩니다.
  늦게 시작해도 남은 걸 다 하면 100%가 됩니다. 놓친 수는 옆에 작게만 적고 리포트에서 정확히 셉니다
- **제한시간을 넘겨도 완료는 축하합니다.** "끝까지 해낸 게 더 멋져" — 끝까지 한 것이 더 중요합니다
- 연속 기록이 끊겨도 나무라지 않습니다. 배지가 조용히 사라질 뿐입니다
- **형제를 나란히 비교하는 화면은 만들지 않았습니다**
- 리포트 맨 위는 "이번 주 잘한 것"입니다. 못한 것을 세는 화면이 되지 않게 했습니다

### 음성

일정 제목을 그대로 읽어 줍니다. 브라우저 정책상 **한 번 눌러 켜야** 소리가 나고,
앱이 열려 있을 때만 울립니다(화면 알림과 같은 한계).

소리는 화면 알림보다 훨씬 빨리 질리므로 제동을 넷 걸었습니다.

| 제동 | 기본값 |
|---|---|
| 하루 최대 횟수 | 8회 |
| 조용한 시간대 | 21:30 ~ 07:00 |
| 같은 일정의 같은 상황 | 하루 한 번만 |
| 문구 돌려 쓰기 | 상황마다 2~3가지 |

칭찬 음성은 **오늘 계획을 다 채웠을 때만** 나옵니다. 하나 끝낼 때마다 말하면 금방 질립니다.
화면 알림과 음성은 따로 켤 수 있어, 알림 권한을 거부했어도 음성만으로 쓸 수 있습니다.

한국어 목소리는 기기에 설치된 것을 씁니다. 없으면 기본 목소리로 읽습니다.
